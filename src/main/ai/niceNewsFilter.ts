...
import type { AppSettings, NiceNewsArticleInput, NiceNewsFilterResult } from '../../shared/types';

export async function filterNiceNewsTitles(
  articles: NiceNewsArticleInput[],
  settings: AppSettings
): Promise<NiceNewsFilterResult> {
  if (articles.length === 0) {
    return { success: true, negativeArticleIds: [] };
  }

  const ruleBasedNegativeIds = new Set(
    articles.filter((article) => isObviouslyNegativeTitle(article.title)).map((article) => article.id)
  );
    ...
    const responseText = await callChatCompletion(settings, [
      {
        role: 'system',
        content:
          'You classify news headlines for a positive RSS reader mode. Respond with valid JSON only.',
      },
      { role: 'user', content: promptContent },
    ], 1200);

    let parsed: { negativeArticleIds: string[] };
    try {
...
function parseNiceNewsResponse(text: string): { negativeArticleIds: string[] } {
  const cleaned = cleanJson(text);
  const parsed = JSON.parse(cleaned) as {
    negativeArticleIds?: unknown;
    negativeIds?: unknown;
    ids?: unknown;
    articles?: unknown;
  } | unknown[];
  ...
  if (Array.isArray(ids)) {
    return {
      negativeArticleIds: ids.filter((id): id is string => typeof id === 'string'),
    };
  }
  ...
  return {
    negativeArticleIds: parsed.negativeArticleIds.filter((id): id is string => typeof id === 'string'),
  };
}
...
function hasNegativeClassification(item: object): boolean {
...
function isObviouslyNegativeTitle(title: string): boolean {
